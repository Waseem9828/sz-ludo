...
import { Game, listenForComputerGames, updateGameStatus } from '@/lib/firebase/games';
import { Loader } from 'lucide-react';
import { updateUserWallet } from '@/lib/firebase/users';

type MatchStatus = 'under_review' | 'completed' | 'cancelled' | 'disputed';

export default function ComputerProPage() {
    const [matches, setMatches] = useState<Game[]>([]);
    const [loading, setLoading] = useState(true);
    const { toast } = useToast();

    useEffect(() => {
        const unsubscribe = listenForComputerGames(
            (games) => {
    ...
    const handleApprove = async (match: Game) => {
        ...
        try {
            ...
            await updateUserWallet(match.winner, finalAmount, 'winnings', 'winnings', `Computer Pro win: ${match.id}`);
          ...
          <TableBody>
            {matches.map((match) => {
              ...
              <TableRow key={match.id}>
                <TableCell>{match.player1?.displayName}</TableCell>
                <TableCell>₹{match.amount}</TableCell>
                <TableCell>{match.winner === match.player1.uid ? match.player1.displayName : 'N/A'}</TableCell>
                <TableCell>
                                     ...
                                     <CardContent className="p-4 text-sm space-y-2">
                                          <div className="flex justify-between">
                                           ...
                                           <div className="flex justify-between">
                                            ...
                                            <span className="font-medium">{match.winner === match.player1.uid ? match.player1.displayName : 'N/A'}</span>
                                          </div>
                                     </CardContent>
                                  </Card>
